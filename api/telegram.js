...
const {
  ...
  markAnswered, declineCount, codeMessage, depositMessage, MISTAKE_LINE, PARTNER_ID, DERIV_PROFILE, DERIV_SIGNUP, EXAMPLE_CLIENT_ID,
} = require("./_lib/ea");
  ...
  if (cmd) {
    ...
    if (repliedTo) {
      ...
      if (named) {
        ...
        if (!reqst) {
          await say(chatId, `Nothing is waiting for a decision for <code>${named[1]}</code>.`, msg.message_id);
