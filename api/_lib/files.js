...
async function storeFile(bytes, name, type) {
  ...
  if (bytes.length > MAX_BYTES) { console.error("[files] too large:", bytes.length); return null; }
  ...
  if (!r.ok) { console.error("[files] upload failed:", r.status, await r.text().catch(() => "")); return null; }
...
async function saveTelegramFile(fileId, fallbackName, declaredType) {
  ...
  try {
    ...
    if (!meta || !meta.ok || !path) { console.error("[files] getFile gave no path"); return null; }
    if ((meta.result.file_size || 0) > MAX_BYTES) { console.error("[files] telegram file too large"); return null; }

    const res = await fetch(`${API}/file/bot${token}/${path}`);
    if (!res.ok) { console.error("[files] download failed:", res.status); return null; }
