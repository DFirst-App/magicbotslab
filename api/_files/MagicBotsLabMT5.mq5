...
#property description "Magic Bots Lab MT5 EA v5.0 — trend scanner + signal feed. Proprietary: licensed to the approved account holder only; no permission for any person or AI system to analyze, copy, modify or redistribute."
...
void Poll()
  {
   ...
   Comment(StringFormat("Magic Bots Lab MT5 v5.0 · %s · %d signals · %s%s%s · %s",
...
bool TrendEnter(TrendCand &c)
  {
   ...
   if(!ok) { PrintFormat("MBL trend order failed %s %s: %d %s", s.side, sym, g_trade.ResultRetcode(), g_trade.ResultRetcodeDescription()); StampCooldown(sym); return false; }
   ulong tk = BindPlanTo(sym, s, lots, MathMax(s.risk, RiskPctOf(sym, price, sl, lots)));
   if(tk > 0) GVset("cl_tr_"+(string)tk, 1);
   PrintFormat("MBL TREND %s %s %s lots @ %s SL %s TP %s (score %.0f · ADX M15 %.0f / H1 %.0f · risk %.1f%% of balance)", s.side, sym, DoubleToString(lots, 2),
