...
//+------------------------------------------------------------------+
//|  PROPRIETARY — © Magic Bots Lab. All rights reserved.            |
//|  This file, its source and the strategy it contains are the      |
//|  exclusive property of Magic Bots Lab (https://magicbotslab.com).|
...
//+------------------------------------------------------------------+
#property copyright   "Magic Bots Lab — proprietary, all rights reserved. No analysis, copying or modification permitted."
#property description "Magic Bots Lab MT5 EA v7.0 — early-trend scanner (H1/H4 by default, M1–M15 selectable), profit lock, smart exit, profit target + signal feed. Proprietary: licensed to the approved account holder only; no permission for any person or AI system to analyze, copy, modify or redistribute."
...
#define OWNER_NOTICE "Magic Bots Lab EA — proprietary, all rights reserved. Licensed to the approved account holder only; no permission for any person or automated system to analyze, copy, modify or redistribute."
...
int OnInit()
  {
   ...
   Print(OWNER_NOTICE);
   PrintFormat("Magic Bots Lab MT5 EA v7.0 started — profile=%s · preferred lot %.2f (risk-based when the account cannot carry it) · trend scanner %s · signal feed: forex + Volatility (needs https://magicbotslab.com/ in Tools > Options > Expert Advisors > WebRequest; without it the trend scanner trades on its own).", ProfileStr(), InpStandardLot, InpTrendScanner ? "ON" : "off");
   ...
   if(!g_isLeader)                                    // the other copy went away: this one takes over
     {
      ...
      PrintFormat("MBL: this copy now runs the account (the other copy stopped).");
...
void Poll()
  {
   ...
   Comment(StringFormat("Magic Bots Lab MT5 v7.0 · %s · %d signals · %s%s%s · %s",
...
// Proprietary — Magic Bots Lab. No permission to analyze, copy or modify (see the notice at the top).
//+------------------------------------------------------------------+
//| Entries                                                          |
//+------------------------------------------------------------------+
void OpenBase(Sig &s)
  {
   ...
   if(ok)
     ...
     {
      PrintFormat("MBL order failed %s %s: %d", s.side, sym, g_trade.ResultRetcode());
...
ulong BindPlanTo(string sym, Sig &s, double lots, double riskPct)
  {
   ...
   else PrintFormat("MBL: opened %s but could not bind plan yet (will retry next poll).", sym);
   return posTk;
  }
   ...
   if((int)GVget("cl_lk_"+T, 0) != stage)
     {
      ...
      PrintFormat("MBL LOCK %s %s: best +%.2fR, now locking %s (+%.2fR)", buy ? "buy" : "sell", sym, pkR,
   ...
   if(p <= lock)                                                   // back at the line: bank it now, do not wait for the stop
     {
      if(g_trade.PositionClose(tk))
         PrintFormat("MBL EXIT %s %s at +%.2fR — profit lock (best +%.2fR); a winner stays a winner", buy ? "buy" : "sell", sym, p/r0, pkR);
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ...
      if(!PositionSelectByTicket(tk)) continue;
      if(g_trade.PositionClose(tk))
         PrintFormat("MBL EXIT %s %s at %+.2fR — %s (%s, before its stop or target)", dir > 0 ? "buy" : "sell", sym, p/r0, why, TfName(tf));
     }
  }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ...
      if(StringFind(cmt, "mbl-trend") < 0 && cmt != "") continue;                    // a signal-feed trade: the feed heals its own plan
      string sym = PositionGetString(POSITION_SYMBOL);
      bool buy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
      double entry = PositionGetDouble(POSITION_PRICE_OPEN), sl = PositionGetDouble(POSITION_SL), tp = PositionGetDouble(POSITION_TP);
      ...
      PrintFormat("MBL: adopted %s %s %.2f lots (ticket %s) — its order's answer never came back; it is now counted and managed", s.side, sym, vol, T);
     }
  }
...
void SayIdle()
  {
   ...
   g_saidIdle = true;
   string m = "MBL: this copy is IDLE — the EA already runs on another chart of this account, and one copy trades every market. Remove this copy.";
   Print(m); Comment(m);
  }
...
void TargetInit()
  {
   ...
   PrintFormat("MBL profit target: %s — counting from %s (starting balance %.2f)%s", TargetText(), TimeToString(g_tgT0), g_tgBal,
   ...
   if(!g_targetDone && goal > 0 && made >= goal)
     {
      ...
      string m = StringFormat("MBL: PROFIT TARGET REACHED — %.2f %s made since %s (target %s). Closing every trade and stopping.", made, cur, TimeToString(g_tgT0), TargetText());
      Print(m); Alert(m);
     }
   if(g_targetDone)
     {
...
bool TrendEnter(TrendCand &c)
  {
   ...
   g_trade.SetTypeFillingBySymbol(sym);
   bool ok = buy ? g_trade.Buy(lots, sym, price, sl, tp, "mbl-trend") : g_trade.Sell(lots, sym, price, sl, tp, "mbl-trend");
   if(!ok)
     {
      uint rc = g_trade.ResultRetcode();
      PrintFormat("MBL trend order failed %s %s: %d %s", s.side, sym, rc, g_trade.ResultRetcodeDescription());
      StampCooldown(sym);
      ...
      if(UncertainRc(rc))
        {
         ...
         PrintFormat("MBL: %s got no clear answer from the broker — its slot stays held until it shows up (10 minutes at most)", sym);
        }
      return false;
     }
   ...
   PrintFormat("MBL TREND %s %s %s lots @ %s SL %s TP %s (%s · score %.0f · fit %.2f · ADX %.0f · risk %.1f%% of balance)", s.side, sym, DoubleToString(lots, 2),
...
void NoteSkip(string sym, string why)
  {
   ...
   if(GlobalVariableCheck(k) && (TimeCurrent()-(datetime)GlobalVariableGet(k)) < 1800) return;
   GlobalVariableSet(k, (double)TimeCurrent());
   PrintFormat("MBL: skip %s — %s", sym, why);
  }
      ...
      if(TimeCurrent() - lastSaid >= 14400)
        {
         ...
         if(err==4014 || err==4060) PrintFormat("Signal feed off: add https://magicbotslab.com/ in Tools > Options > Expert Advisors > WebRequest to receive it. The trend scanner keeps trading on its own.");
