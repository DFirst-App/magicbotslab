  ...
  class SmartRecoveryDifferBot {
    ...
    analyzeMarketsForRecovery() {
      ...
      /* Fallback: no market has a strong bias, so take the one with the most
         ...
         other way. And since a 60% bias is rare on a volatility index, this
         was the branch that ran nearly every time: the recovery was Over 4
         almost always, whatever the digits said. */
      if (!bestMarket) {
        let maxTicks = 0;
        this.markets.forEach(market => {
          const analysis = this.marketAnalysis[market];
