...
create table if not exists public.mbl_ea_requests (
...
create unique index if not exists mbl_ea_requests_code_idx
  on public.mbl_ea_requests (code) where code is not null;

create unique index if not exists mbl_ea_requests_tg_idx
  on public.mbl_ea_requests (tg_message_id) where tg_message_id is not null;

create index if not exists mbl_ea_requests_visitor_idx
  on public.mbl_ea_requests (visitor_id, created_at desc);

create index if not exists mbl_ea_requests_open_idx
  on public.mbl_ea_requests (created_at desc)
  where status = 'pending' and answered_at is null;

alter table public.mbl_ea_requests enable row level security;
