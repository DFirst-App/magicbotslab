/**
 * MAGIC BOTS LAB — a service worker that caches nothing.
 *
 * It exists for one reason: Chrome will not offer to install a site that has
 * no service worker with a fetch handler. The manifest and the icons have been
 * here all along, so the "Install app" button was being drawn for a prompt
 * that was never going to fire. This is that handler and no more.
 *
