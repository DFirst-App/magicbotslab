...
export function useAccumulatorTrading({ ws, isConnected, isExhausted, isAuthenticated, onAuthWSFailed }: UseAccumulatorTradingParams): UseAccumulatorTradingReturn {
  ...
  const [growthRate, setGrowthRate] = useState<GrowthRate>(0.05);
  ...
  const { proposal, proposalError } = useAccumulatorProposal(tradingWs, tradingIsConnected, proposalParams);

  const buyContract = useCallback(async () => {
    if (!proposal) {
      const reason = proposalError
        ? `Cannot buy: proposal has error → ${proposalError}`
        : 'Cannot buy: no valid proposal available (parameters may be invalid or proposal expired)';
      console.error('[ACCU Buy] Blocked buy attempt', {
        reason,
        currentParams: proposalParams,
        activeSymbol: activeSymbol?.underlying_symbol,
        stake,
        growthRate,
      });
      return;
    ...
    // Rich log before every buy attempt — user can copy this when reporting issues
    console.log('[ACCU Buy] Attempting buy with proposal', {
      ...
      hasCrossedBarrier: proposal.hasCrossedBarrier,
    });

    await buyWithProposal(proposal);
  }, [proposal, proposalError, buyWithProposal, proposalParams, activeSymbol, stake, growthRate]);
