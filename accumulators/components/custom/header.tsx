...
interface HeaderProps {
  ...
  /** Magic Bots Lab: hide login/signup and use site session instead. */
  embeddedMode?: boolean;
  /** Back link shown in embedded mode. */
  backHref?: string;
}
...
export function Header({
  ...
  actions,
  embeddedMode = false,
  backHref = '../trading-dashboard.html',
}: HeaderProps) {
    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 border-b bg-background/80 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        {embeddedMode && (
          <a
            href={backHref}
            className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
          >
      ...
      <div className="flex items-center gap-3">
        ...
        {isAuthenticated ? (
          ...
          <Button variant="outline" size="sm" disabled={isAuthenticating}>
