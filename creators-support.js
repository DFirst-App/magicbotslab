 ...
 *   window.MBL_SUPPORT_SOURCE    — label shown in Telegram for this page
 *   window.MBL_SUPPORT_GREETING  — the first line in the panel
 *   window.MBL_SUPPORT_IDENTITY(name, email)
 *   window.MBL_SUPPORT_ASK({ name, email, text })  — fill, add as sent, open
 *   window.MBL_SUPPORT_ID        — the browser id a code or reply is bound to
 */
(function () {
  "use strict";
   ...
   *  sent on the page's behalf — the MT5 EA request does, so the person opens
  ...
  window.MBL_SUPPORT_REPLY_IDS = function () {
        ...
        state.thread.map(function (l) {
          if (l.from === "us") {
            ...
            return '<div class="sup-reply">' +
              '<div class="sup-reply-who"><i></i>Magic Bots Lab support' +
  ...
  function mergeReplies(reps) {
    ...
    try { window.dispatchEvent(new CustomEvent("mbl:support-reply")); } catch (e) {}
  }
  ...
  function send() {
    ...
    (file ? readFileAsBase64(file) : Promise.resolve(null)).then(function (data) {
      ...
      try { window.dispatchEvent(new CustomEvent("mbl:support-sent", { detail: { kind: kindSent } })); } catch (e) {}
    }).catch(function () {
      state.busy = false;
